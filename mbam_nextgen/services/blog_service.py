...
class BlogService:
    ...
    async def insert_place_map(self, frame, query: str) -> bool:
        """SE3 에디터에 네이버 장소(지도) 컴포넌트를 삽입한다. query=장소명/주소.
        흐름: 장소 버튼 → 검색 입력 → 검색 → 첫 결과 '추가' → 확인.
        (셀렉터는 SE3 진단으로 확인: se-map-toolbar-button / react-autosuggest__input /
         se-place-search-button / se-place-map-search-result-item / se-place-add-button /
         se-popup-button-confirm)"""
        if not query or not query.strip():
            return False
        query = query.strip()
        try:
            logger.info(f"[BlogService] 🗺️ 장소(지도) 삽입 시도: {query}")
            ...
            await asyncio.sleep(2)
            logger.info("[BlogService] ✅ 장소(지도) 삽입 완료")
            return True
        except Exception as e:
            logger.error(f"[BlogService] 장소 삽입 실패: {e}")
            try:
