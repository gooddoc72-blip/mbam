...
async def _naver_blog_search(query: str, display: int = 5) -> list:
    ...
    return [{"title": _clean(it.get("title")), "desc": _clean(it.get("description"))} for it in items]
...
async def collect_matjip_source(place_url: str = "", keyword: str = "", log=None, browser_ok: bool = True) -> dict:
    ...
    if blog_query:
        if browser_ok:
            ...
            # 클라우드 서버: 공식 블로그 검색 API(브라우저 불필요, 스니펫)
            try:
                _log(f"'{blog_query}' 블로그 후기(공식 검색) 수집 중...")
                items = await _naver_blog_search(blog_query, display=5)
                n = 0
                for it in items:
                    snippet = (it.get("desc") or "").strip()
                    if snippet:
                        parts.append(f"[블로그 후기: {it.get('title', '')}]\n{snippet}")
                        n += 1
                _log(f"블로그 후기(공식) {n}건 수집")
            except Exception as e:
